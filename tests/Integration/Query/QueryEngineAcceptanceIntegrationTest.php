...
{
    public function testStreamingQueryKeepsMemoryBoundedOnLargeSqliteFixture(): void
    {
    ...
    public function testTransactionalRollbackIsExercisedThroughQueryExecutor(): void
    {
        $connection = $this->sqliteConnection();
    ...
    public function testPaginatorUsesApproximateTableStatusRowsAgainstSqlite(): void
    {
        $connection = $this->sqliteConnection();
    ...
    public function testDelimiterBatchCreatesRoutineAgainstMysqlWhenEnabled(): void
    {
        if (getenv('SQLCRAFT_RUN_ENGINE_INTEGRATION') !== '1') {
            self::markTestSkipped('Set SQLCRAFT_RUN_ENGINE_INTEGRATION=1 with MySQL running.');
        }

        $connection = (new \SQLCraft\Driver\MySQLDriver(
            new \SQLCraft\Connection\PdoConnectionFactory(new PdoExceptionTranslator()),
            new MySQLPlatform(),
